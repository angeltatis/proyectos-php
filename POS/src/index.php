...
$compras = mysqli_query($conexion, "SELECT * FROM compras WHERE fecha > CURDATE()");
$total_compras['compras'] = mysqli_num_rows($compras);
session_start();
    ...
    <div class="col-lg-3 col-md-6 col-sm-6">
        <div class="card card-stats">
            <div class="card-header card-header-info card-header-icon">
                ...
                <a href="compras.php" class="card-category text-info font-weight-bold">
                    compras
                </a>
                <h3 class="card-title"><?php echo $total_compras['compras']; ?></h3>
            </div>
            <div class="card-footer bg-danger text-white">
            </div>
        </div>
